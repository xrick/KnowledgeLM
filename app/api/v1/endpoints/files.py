...
def get_effective_user_id(user_id: Optional[str]) -> str:
    """
    Get effective user_id, using default if not provided.

    Args:
        user_id: User ID from header (optional)

    Returns:
        Effective user_id (provided or default)

    Raises:
        HTTPException: If user_id required but not provided and default disabled
    """
    if user_id is None:
        if settings.DEFAULT_USER_ENABLED:
            logger.info(f"Using default user_id: {settings.DEFAULT_USER_ID}")
            ...
            raise HTTPException(
                ...
                detail={
                    ...
                    "details": {"hint": "Provide X-User-ID header with a valid UUID v4"}
                }
            )
...
async def list_user_files(
    user_id: Optional[str] = Header(None, alias="X-User-ID", description="User UUID (optional, uses default if not provided)"),
    ...
    """
    try:
        # Get effective user_id (provided or default)
        ...
        logger.error(f"Failed to list files for user {effective_user_id if 'effective_user_id' in dir() else user_id}: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail={
                "error": "DatabaseError",
...
async def delete_file(
    ...
    try:
        ...
        if file_data.get("user_id") != effective_user_id:
            logger.warning(
                f"Authorization failed: User {effective_user_id} attempted to delete file {file_id} "
        ...
        logger.info(f"File deleted successfully: {file_id} by user {effective_user_id}")
